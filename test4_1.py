...
example4（microscope.seq）に対応するテスト。

機能:
  (1) .seq を読み込み test1_1 形式（SurfaceSpec/ReportConfig/SeqConditions）に変換
  (2) YAML で保存
  (3) YAML を読み込み直して復元（確認用）
  (4) draw_system / grid_generator / spotdiagram / Ray_fan を実行（example4相当）
...
outdir = os.path.join("out", "examples", "test4_1")
...
# -------------------------------------------------
seq_path = os.path.join("ex_CodeV", "microscope.seq")
doc = parse_seq_file(seq_path)

cfg = build_report_config_from_seq(doc)
# example4 はスクリプト側で FNO を 0.7 にしていたので fallback=0.7
field_angles_deg = pick_field_angles_deg(doc.conditions)
fno = pick_fno(doc.conditions, fallback_fno=0.7)

# -------------------------------------------------
# (2) Save YAML
# -------------------------------------------------
dump_seq_document(os.path.join(yamldir, "microscope.doc.yaml"), doc)
dump_surface_specs(os.path.join(yamldir, "microscope.specs.yaml"), doc.prescription)
dump_seq_conditions(os.path.join(yamldir, "microscope.cond.yaml"), doc.conditions)
dump_report_config(os.path.join(yamldir, "microscope.cfg.yaml"), cfg)

# -------------------------------------------------
# (3) Load YAML back (sanity / confirmation)
# -------------------------------------------------
doc2 = load_seq_document(os.path.join(yamldir, "microscope.doc.yaml"))
cfg2 = load_report_config(os.path.join(yamldir, "microscope.cfg.yaml"))
...
rename_fixed_png(
    ...
    outdir,
    tag="test4_1_grid_n12",
)
analysis.spotdiagram(L, [1, 2, 3], [1, 2, 3], n=12, grid_type="grid")
rename_latest_spotdiagram(outdir, tag="test4_1_spot_grid_n12")
