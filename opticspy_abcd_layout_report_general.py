...
# -----------------------------------------------------------------------------
# Paraxial math utilities (ray vector [y, theta])
# -----------------------------------------------------------------------------
def translation_matrix(t: float) -> np.ndarray:
...
def compute_abcd_from_prescription(
    ...
    Propagation:
      for i = start..end:
        ...
        - if i < end: translate by thickness t_i
    """
...
def compute_report(cfg: ReportConfig) -> Dict[str, Any]:
    ...
    if cfg.image_surface is not None and cfg.image_surface in z:
        ...
        M_to_img = translation_matrix(dz) @ M
